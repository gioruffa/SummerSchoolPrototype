...
#our implementation uses this to separate numeric(float) from string data
    try:
        float(msg.payload)
        es.index(index="customer", doc_type="numeric", body={"topic" : msg.topic, "dataFloat" : float(msg.payload), "timestamp": datetime.utcnow()})

    except:
        # print("FUCKING TYPE!")
        # print(type(msg.topic))
        es.index(index="customer", doc_type="string", body={"topic" : msg.topic, "dataString" : msg.payload, "timestamp": datetime.utcnow()})
...
client.username_pw_set("test", password="summerschool")
